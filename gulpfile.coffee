...
paths =
  jekyll: ["#{dirs.jekyll}/**/*.{html,markdown,md}"]
...
gulp.task 'jekyll', (cb) ->
  ...
  jekyllCommand = 'jekyll <%= options.subCommand %> --config <%= options.configFiles %>'
  command = gutil.template jekyllCommand, {file: null, options: jekyllOptions}
...
gulp.task 'build', ->
  env = 'production'
  runSequence 'clean', 'prodJade', 'jekyll', ['prodJS', 'prodCSS', 'image']
